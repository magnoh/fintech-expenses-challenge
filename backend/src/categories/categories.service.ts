...
export class CategoriesService {
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    try {
      ...
      // Se houver restrição de chave estrangeira (RESTRICT) no banco de dados
      if (error.code === '23503') {
        throw new ConflictException('Não é possível excluir esta categoria pois ela possui transações associadas.');
      }
      throw error;
    }
  }
